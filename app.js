...
app.get('/orcamentos', (request, response) => {
	...
	db.query(query, (error, results) => {
		if(error){
			...
			cliente = results
			query = "select c.nome as nome, DATE_FORMAT(o.data, '%d/%m/%Y') as data from orcamento o, cliente c where o.id_cliente = c.id_cliente and o.id_conta = '"+id_conta+"'"
			db.query(query, (error, results) => {
				if(error){
					response.send('Erro: ' + error)
				}else{
...
app.get('/orcamento-print', (request, response) => {
	...
	let query = "select DATE_FORMAT(o.data, '%d/%m/%Y') as data, i.qtd, i.unitario, i.total, m.nome as nome_produto, format(i.unitario, 2, 'de_DE') as unitario, format(i.total, 2, 'de_DE') as total, c.nome as nome_cliente, c.endereco, c.telefone, o.observacao from orcamento o inner join orcamento_item i on o.id_orcamento = i.id_orcamento inner join cliente c on o.id_cliente = c.id_cliente inner join material m on i.id_material = m.id_material where o.id_orcamento = 8" //'"+id_orcamento+"'"
	db.query(query, (error, results) => {
		if(error){
			response.send('Erro: ' + error)
		}else{		
